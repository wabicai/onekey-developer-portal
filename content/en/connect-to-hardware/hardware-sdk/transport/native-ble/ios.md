...
In JS, you would call these handlers from your adapter to mirror the demo's behavior. Otherwise, you can handle UI entirely in JS using `UI_EVENT` — see [Config Event](../../api-reference/config-event) for event wiring and responses (WebUSB guide includes minimal dialogs).
...
- Always subscribe to `UI_EVENT` in JS to avoid stalled requests (see [Config Event](../../api-reference/config-event)).
- Keep `web/web_dist/` in your app bundle and adjust the `load` path accordingly.

## References

- Message Protocol (64‑byte framing): [OneKey Message Protocol](./onekey-message-protocol)
- Low‑level transport plugin contract: [Low-level Transport Plugin](./low-level-transport-plugin)
