...
3. PIN — Listen for `UI_EVENT` and answer `REQUEST_PIN` via `uiResponse` (prefer on‑device entry). See [Config Event](../api-reference/config-event).
...
- WebUSB (browser) → [WebUSB Connection Guide](../transport/web-usb)
- iOS BLE (native) → [iOS BLE](../transport/native-ble/ios)
- Android BLE (native) → [Android BLE](../transport/native-ble/android)
- React Native BLE (pure RN) → [React Native BLE](../transport/react-native-ble)
