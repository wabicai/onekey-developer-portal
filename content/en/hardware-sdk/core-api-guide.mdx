...
- Subscribe:
  - Bus: `HardwareSDK.on(UI_EVENT, cb)` → `{ event, type, payload }`
  - Specific: `HardwareSDK.on(UI_REQUEST.REQUEST_PIN, cb)` → payload only
- Common request → response:
  - PIN: `UI_REQUEST.REQUEST_PIN` → `UI_RESPONSE.RECEIVE_PIN`
  - Passphrase: `UI_REQUEST.REQUEST_PASSPHRASE` → `UI_RESPONSE.RECEIVE_PASSPHRASE`
  - Device select (Bootloader/Web): `UI_REQUEST.REQUEST_SELECT_DEVICE` / `..._IN_BOOTLOADER_FOR_WEB_DEVICE` → `UI_RESPONSE.SELECT_DEVICE_IN_BOOTLOADER_FOR_WEB_DEVICE`
- Timing:
  - Call `uiResponse` only after the request event fires.
  - Cancellation/timeout: allow user cancel in UI and surface an error; re-initiate if needed.
  - Unsubscribe with `HardwareSDK.off(...)` when leaving the page to avoid duplicate handlers.
