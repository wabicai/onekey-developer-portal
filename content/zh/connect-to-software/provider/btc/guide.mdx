...
* 提供两种类型的 API：`window.unisat` 和 `window.$onekey.btc`。
* 推荐使用 `window.$onekey.btc`。
...
* 获取用户账户信息：[`provider.getAccounts`](api-reference/getaccounts)
* 发送付款：[`provider.sendBitcoin`](api-reference/sendbitcoin)
