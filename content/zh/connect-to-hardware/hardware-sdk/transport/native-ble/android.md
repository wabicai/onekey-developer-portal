...
在您的 JS 包中处理 `UI_EVENT` 并使用 `HardwareSDK.uiResponse` 响应。参见 [事件配置](../../api-reference/config-event) 了解事件连接，以及 WebUSB 指南了解最小的、生产就绪的对话框。
...
- 消息协议（64 字节帧）：[OneKey 消息协议](./onekey-message-protocol)
- 底层传输插件契约：[底层传输插件](./low-level-transport-plugin)
