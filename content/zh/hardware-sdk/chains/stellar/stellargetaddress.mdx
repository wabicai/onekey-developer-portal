...
* `path` - _必需_ `string | Array<number>` **长度为 `3`，这是一个特殊的链。** [更多信息](../../../api-reference/path)
...
返回一批 Stellar 地址而不在设备上显示：

```typescript
HardwareSDK.stellarGetAddress(connectId, deviceId, {
    bundle: [
        { path: "m/44'/148'/0'", showOnOneKey: false }, // 账户 1
        { path: "m/44'/148'/1'", showOnOneKey: false }, // 账户 2
        { path: "m/44'/148'/2'", showOnOneKey: false }  // 账户 3
    ]
});
```
