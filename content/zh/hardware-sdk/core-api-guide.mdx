...
- 事件组：`UI_EVENT`、`DEVICE_EVENT`、`FIRMWARE_EVENT`。
- 订阅方式：
  - 总线：`HardwareSDK.on(UI_EVENT, cb)` → `{ event, type, payload }`
  - 指定：`HardwareSDK.on(UI_REQUEST.REQUEST_PIN, cb)` → 直接拿 payload
- 常见请求 → 响应：
  ...
  - 设备选择（Bootloader/Web）：`UI_REQUEST.REQUEST_SELECT_DEVICE` / `..._IN_BOOTLOADER_FOR_WEB_DEVICE` → `UI_RESPONSE.SELECT_DEVICE_IN_BOOTLOADER_FOR_WEB_DEVICE`
- 时序：只能在事件触发后调用 `uiResponse`；取消/超时要在 UI 提示，并可重新发起；离开页面时用 `HardwareSDK.off(...)` 解绑，避免重复处理。
